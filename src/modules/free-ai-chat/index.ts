...
function validateUUID(id: string, paramName = 'id'): void {
  ...
  .get(
    ...
    async ({ query, user, anonymousIdHash, set }: any) => {
      ...
      if (user) {
        ...
        return { sessions, total: Number(countRow?.count ?? 0) };
      }

      if (anonymousIdHash) {
        const [sessions, total] = await Promise.all([
          trialService.listTrialSessionsByHash(anonymousIdHash, limit, offset),
          trialService.getTrialSessionCountByHash(anonymousIdHash),
        ]);
        return {
  ...
  .get(
    ...
    async ({ params, query, user, anonymousIdHash, set }: any) => {
      ...
      if (user) {
        ...
        return { messages: messages.reverse(), total: Number(countRow?.count ?? 0) };
      }
