...
export const vocabularyModule = withAdmin(
  withOptionalAuth(new Elysia({ prefix: '/api/vocabulary' })),
)
  // ==================== Vocabulary CRUD ====================
  ...
  .get(
    ...
    async ({ query, set }: any) => {
      ...
      if (!item) {
        set.status = 404;
        return { error: 'Not Found', message: 'No vocabulary item found matching filters' };
      }
      return { data: item };
    },
    {
  ...
  // POST /api/vocabulary/bulk - Bulk create vocabulary items (admin only)
  .post(
    '/bulk',
    async ({ body, set }: any) => {
      ...
      const inserted = await db.insert(vocabularyItem).values(values).returning();
      set.status = 201;
    ...
    },
    {
      admin: true,
      body: t.Object({
        items: t.Array(createVocabularySchema, { minItems: 1 }),
      }),
      detail: {
        tags: ['Vocabulary'],
        summary: 'Bulk create vocabulary items',
        description: 'Admin only. Create multiple vocabulary items at once.',
      },
    },
  )
